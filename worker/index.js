...
//
// 部署前需要：
// 1. 创建 D1 数据库，执行设计文档 4.0 节的建表 SQL
// 2. 在 Worker 设置中绑定 D1 数据库，变量名必须为 DB
// 3. 配置以下 Secrets / 环境变量：
// - LOGIN_PASSWORD 网页登录密码
// - SESSION_SECRET session token 签名密钥（任意长随机字符串）
// - GITHUB_TOKEN 具有 repo 权限的 GitHub PAT，用于 repository_dispatch
// - GITHUB_REPO 格式 "owner/repo"，存放 workflow 的 public 仓库
// - REPORT_TOKEN workflow 上报专用密钥，需与 Actions Secrets 中一致
...
function json(data, status = 200, extraHeaders = {}) {
  ...
  async function createSessionToken(env) {
    ...
    // 修复新问题A：SESSION_SECRET 未配置时，用 LOGIN_PASSWORD 做 HMAC 派生，而非直接用密码本身作签名密钥，避免密码即密钥的简称换
    const secret = env.SESSION_SECRET || (env.LOGIN_PASSWORD ? `derived:${env.LOGIN_PASSWORD}` : 'fallback-secret');
  ...
  function requireReportToken(request, env) {
    const token = request.headers.get('X-Report-Token');
    if (!token || !env.REPORT_TOKEN) return false;
    return timingSafeEqual(token, env.REPORT_TOKEN);
  }
  ...
  async function dispatchGithubWorkflow(env, buildId) {
    const resp = await fetch(`https://api.github.com/repos/${env.GITHUB_REPO}/dispatches`, {
      ...
      body: JSON.stringify({ event_type: 'build-openwrt', client_payload: { build_id: buildId } }),
    });
  ...
  async function handleLogin(request, env) {
    ...
    if (!env.LOGIN_PASSWORD) {
      return err('服务器未配置 LOGIN_PASSWORD', 500);
    }
    if (!timingSafeEqual(body.password, env.LOGIN_PASSWORD)) {
      return err('密码错误', 401);
    }
  ...
  * { box-sizing: border-box; }
  ...
  const store = { authed: false, route: { name: 'templates', params: {} }, templates: [], builds: [], pollTimer: null };
  ...
  function renderTopbar(activeName) {
    ...
    <div class="topbar">
      ...
      <div class="right">
        ...
        <button class="ghost small" id="logout-btn">退出登录</button>
      </div>
    </div>\`;
  }
  ...
  function bindTopbar(root) {
    ...
    qs(root, '#logout-btn')?.addEventListener('click', async () => {
  ...
  async function handleApiRequest(request, env, url) {
    ...
    if (path === '/api/report' && method === 'POST') {
      if (!requireReportToken(request, env)) return err('unauthorized', 401);
